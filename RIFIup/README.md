...
Illustration of the proposed method. (a) is the training process of RIFIup. CLIP is frozen and only RIFIup is useful in reasoning. (b) is the reasoning process of SegEarth-OV. The LR feature maps from CLIP are upsampled by RIFIup and then the \texttt{[CLS]} token is subtracted to alleviate global bias. For better presentation, the color renderings follow [FeatUp](https://github.com/mhamilton723/FeatUp).

------

This is a sub-project of [SegEarth-OV](https://github.com/likyoo/SegEarth-OV) to **build** a CUDA version of JBU operation (provided by [FeatUp](https://github.com/mhamilton723/FeatUp)) and **train** RIFIup.
...
If you want to train a your own RIFIup, you'll need to download the Million-AID dataset.
...
Change ``pytorch_data_dir`` in the [config](https://github.com/likyoo/RIFIup/blob/main/featup/configs/upsampler_aid.yaml) file to your data path. We use 2 GPUs with a batch size of 4 on each GPU by default.
