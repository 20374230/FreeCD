...
class SegEarthSegmentation(BaseSegmentor):
    def __init__(self,
        ...
        with torch.no_grad(): # sub_imagenet_template, openai_imagenet_template
            for qw in query_words:
                if self.clip_type == 'BLIP':
                    query =self.net.tokenizer([temp(qw) for temp in sub_imagenet_template], padding='max_length',
                    ...
                    query = self.tokenizer([temp(qw) for temp in sub_imagenet_template]).to(device)
    ...
    def forward_feature(self, img, logit_size=None):
        ...
        if self.feature_up:
            ...
            image_features = image_features.permute(0, 2, 1).view(batch, self.feat_dim, feature_w, feature_h)
            with torch.cuda.amp.autocast():
                image_features = self.upsampler(image_features, img).half()
            image_features = image_features.view(batch, self.feat_dim, image_w * image_h).permute(0, 2, 1)
