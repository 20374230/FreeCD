...
class JBUStack2(torch.nn.Module):

    def __init__(self, feat_dim, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.up1 = JBULearnedRange(3, feat_dim, 32, radius=5)
        self.up2 = JBULearnedRange(3, feat_dim, 32, radius=5)
        self.up3 = JBULearnedRange(3, feat_dim, 32, radius=5)
        self.up4 = JBULearnedRange(3, feat_dim, 32, radius=5)
        self.fixup_proj = torch.nn.Sequential(
            torch.nn.Dropout2d(0.2),
            torch.nn.Conv2d(feat_dim, feat_dim, kernel_size=1))
...
def get_upsampler(upsampler, dim):
    if upsampler == 'bilinear':
        ...
        return JBUStack(dim)
    elif upsampler == 'jbu_stack2':
        return JBUStack2(dim)
