...
class FNOFeatUp(pl.LightningModule):
    def __init__(self,
                 model_type,
                 activation_type,
                 n_jitters,
        ...
        if self.rec_img_weight!=0:
            ...
            #     torch.nn.Tanh()
            # )
            self.projection_img=ResolutionInvariantFNO(
                    input_channels=self.dim,
                    output_channels=3,
                    width=64,
                    modes=8
                )
            if fno_pretrained_ckpt:
                state_dict = torch.load(fno_pretrained_ckpt, map_location='cpu')['state_dict']
                new_state_dict = {
                    k.replace('FNO.', ''): v
                    for k, v in state_dict.items()
                    if k.startswith('FNO.')
                }
                self.projection_img.load_state_dict(new_state_dict, strict=True)
                for p in self.projection_img.parameters():
                    p.requires_grad = False
...
@hydra.main(config_path="configs", config_name="upsampler_aid_FNO.yaml")
def my_app(cfg: DictConfig) -> None:
    ...
    model = FNOFeatUp(
        ...
        chkpt_dir=chkpt_dir,
        fno_pretrained_ckpt=cfg.get('fno_pretrained_ckpt'),
    )
