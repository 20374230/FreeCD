...
class DINOv2Featurizer(nn.Module):
    ...
    def __init__(self, arch, patch_size, feat_type):
        ...
        self.n_feats = 128
        # Load model without pretrained weights first, then load weights manually
        self.model = torch.hub.load('/hdd1/zys/zys/.cache/torch/hub/facebookresearch_dinov2_main/', 'dinov2_vitl14_reg', source='local', pretrained=False)
        # Load pretrained weights from local file
        state_dict = torch.load('/hdd1/zys/zys/.cache/torch/hub/checkpoints/dinov2_vitl14_reg4_pretrain.pth', map_location='cpu')
